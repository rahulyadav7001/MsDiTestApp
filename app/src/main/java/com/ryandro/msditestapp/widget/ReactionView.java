package com.ryandro.msditestapp.widget;
...
public class ReactionView extends View {
    ...
    private void animateEmotionPosition(Emotion emotion) {
        ...
        emotions.get(0).x = RoundedBoard.LEFT + AppConstant.HORIZONTAL_SPACING;
        emotions.get(emotions.size() - 1).x =
                RoundedBoard.LEFT + RoundedBoard.WIDTH - AppConstant.HORIZONTAL_SPACING
                        - emotions.get(emotions.size() - 1).size;

        for (int i = 1; i < selectedIndex; i++) {
            emotions.get(i).x = emotions.get(i - 1).x + emotions.get(i - 1).size
                    + AppConstant.HORIZONTAL_SPACING;
        }
        ...
        if (selectedIndex > 0) {
            emotions.get(selectedIndex).x = emotions.get(selectedIndex - 1).x
                    + emotions.get(selectedIndex - 1).size + AppConstant.HORIZONTAL_SPACING;
        }
    }
    ...
    private class TranslationAnimation extends Animation {
        ...
        private void prepareEmotions() {
            for (int i = 0; i < emotions.size(); i++) {
                emotions.get(i).endAnimatedY = RoundedBoard.TOP + AppConstant.VERTICAL_SPACING;
                ...
                emotions.get(i).startAnimatedX
                        ...
                        + AppConstant.HORIZONTAL_SPACING + (Emotion.MEDIUM_SIZE / 2)
                        : emotions.get(i - 1).x + Emotion.MEDIUM_SIZE + AppConstant.HORIZONTAL_SPACING;
            }
        }
